# Slash and Dice

### About

Slash and Dice is a fanmade guide/toolbox/community made for the mobile game I like so much called Dungeon Slasher.

Feel free to also visit this wiki website, it has a lot of useful info:
[Dungeon Slasher Wiki](https://dungeonslasher.wiki/)

### Tech

This project is built with [Fresh](https://fresh.deno.dev/), a web framework for Deno.

### Development

To start a development server, make sure you have [Deno](https://deno.com/) installed, then run:

```
deno task dev
```

### Contributing

Contributions are welcome! Feel free to open issues or submit pull requests on the GitHub repository.

### Support or other requests

You can reach me at my discord: `xiayunn`.