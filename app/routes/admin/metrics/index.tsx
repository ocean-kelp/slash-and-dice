...
import {
  getCachedMetrics,
  getRecentErrorSummaries,
  getRecentRequests,
} from "@/middlewares/monitoring.ts";
import { appConfig } from "@/utilities/config.ts";
import RequestDetails from "./(_islands)/RequestDetails.tsx";
...
type MetricsSummary = {
  ...
  avgDuration: number;
  slowestRequest: { path: string; duration: number; timestamp: string } | null;
  errorRate: number;
...
type MetricsPageData = {
  ...
  recentErrorSummaries: ErrorSummary[];
...
export const handler = define.handlers({
  GET(ctx) {
    ...
    const { summary, lastRefresh } = getCachedMetrics(100, force);
    const recentRequests = getRecentRequests(20);
    const recentErrorSummaries = getRecentErrorSummaries(100);
    const maxStoredRequests = appConfig.metricsMaxRequests;

    // preserve existing query params (including admin key) when forcing
    const forceUrl = new URL(ctx.req.url);
    forceUrl.searchParams.set("force", "1");
    const forceHref = forceUrl.pathname + forceUrl.search;
...
export default function MetricsPage({ data }: PageProps<MetricsPageData>) {
  const {
    ...
    recentRequests,
    recentErrorSummaries,
    maxStoredRequests: _maxStoredRequests,
    lastRefresh,
    forceHref,
  } = data;
  return (
    <main class="min-h-screen py-12 px-6">
      ...
      <div class="bg-gray-800 rounded-lg p-6 border border-gray-700 mt-8">
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-left text-sm">
            <thead class="border-b border-gray-700">
              <tr>
                <th class="pb-2 text-gray-400 font-medium">ID</th>
                <th class="pb-2 text-gray-400 font-medium">Method</th>
                <th class="pb-2 text-gray-400 font-medium">Path</th>
                <th class="pb-2 text-gray-400 font-medium">Status</th>
                <th class="pb-2 text-gray-400 font-medium">Duration</th>
            ...
            <tbody>
              {recentErrorSummaries.map((err: ErrorSummary) => (
                <tr
                  key={err.id}
                  class="border-b border-gray-700/50 hover:bg-gray-700/30 cursor-pointer"
                  data-id={err.id}
                >
                  <td class="py-2 font-mono text-xs">{err.id}</td>
                  <td class="py-2 text-blue-400 font-mono">{err.method}</td>
                  <td class="py-2 text-gray-300 text-xs">{err.path}</td>
                  <td class="py-2">
                    <span
                      class={`font-semibold ${
                        err.status >= 500 ? "text-red-400" : "text-yellow-400"
                      }`}
                    >
                      {err.status}
                    </span>
                  </td>
                  <td class="py-2">{err.duration}ms</td>
        ...
        <p class="text-xs text-gray-400 mt-2">
          Click a row to view full request details (loaded on demand).
        </p>
      </div>
      ...
      {/* Footer */}
      <div class="mt-8 text-center text-gray-500 text-sm space-y-2">
        <p>
          Metrics are stored in-memory (last {data.maxStoredRequests} requests).
        </p>
        <p>
          Last refresh (UTC):{" "}
          {lastRefresh ? new Date(lastRefresh).toUTCString() : "Never"}
        </p>
        <div class="flex items-center justify-center gap-3">
          <a
            href={forceHref ?? "?force=1"}
            class="inline-block bg-ocean-deep-400 hover:bg-ocean-deep-300 text-white px-3 py-1 rounded"
          >
            Force Refresh
          </a>
          <span class="text-gray-400 text-xs">(forces cache recompute)</span>
        </div>
        </div>
      </div>
    </main>
